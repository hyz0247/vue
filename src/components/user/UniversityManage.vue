  ...
  <div>
    <div style="background: white;padding: 5px;border-radius: 10px;border: 1px solid black;box-sizing: border-box">
      ...
      <el-table :data="tableData"
        ...
        <el-table-column prop="affiliation" label="二级学院账号" align="center">
          <template slot-scope="scope">
          <el-button  type="primary" @click="lookAffiliation(scope.row)" style="width: 54px;height: 30px;align-items: center">查看</el-button>
          </template>
        </el-table-column>
        ...
        <div style="width: 100%;height: 65vh">
          ...
          <el-descriptions :column="1" size="50" border style="width: 35%;float: right;margin-right: 130px">
            ...
            <el-descriptions-item >
              ...
              {{this.univerInfo.address}}
            </el-descriptions-item>
            <el-descriptions-item >
              <template slot="label"><i class="el-icon-s-custom"></i>电子邮箱</template>
              {{this.univerInfo.email}}
            </el-descriptions-item>
            <el-descriptions-item class="long-content">
              <template slot="label"><i class="el-icon-s-custom" ></i>学校简介</template>

                <div>{{ this.univerInfo.description }}</div>
      ...
      <el-dialog
          ...
          width="80%"
          center>
        <el-table :data="collegeTableData"
                  style="width: 100%;font-size: 15px;border-radius: 5px;margin-top: 10px"
                  :header-cell-style="{background: 'lightblue' ,color: '#555555' }">
          <el-table-column prop="username" label="用户名" align="center">
          </el-table-column>
          ...
          <el-table-column prop="contactName" label="联系人名字" align="center">
          </el-table-column>
          ...
          <el-table-column prop="operate" label="操作" align="center">
            <template slot-scope="scope">
              <el-button type="success" title="编辑"  icon="el-icon-edit" @click="modify(scope.row)" circle></el-button>
              <el-button title="删除" type="danger" icon="el-icon-delete" @click="deleteById(scope.row.id)" circle></el-button>
              <el-button type="primary" @click="lookCollegeInfo(scope.row)" style="width: 54px;height: 30px;align-items: center">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-dialog>
      <!--学院详细信息-->
      <el-dialog
          :visible.sync="collegeInfoDialogVisible"
          width="50%"
          center>
        <label style="margin-left: 385px;font-size: 30px">学生信息</label>
        <div style="width: 100%;height: 65vh">
          <el-descriptions :column="1" size="50" border style="width: 35%;float: left">
            <el-descriptions-item >
              <template slot="label" ><i class="el-icon-s-custom"></i>联系人姓名</template>
              {{this.collegeInfo.contactName}}
            </el-descriptions-item>
            <el-descriptions-item >
              <template slot="label"><i class="el-icon-s-custom"></i>联系人电话</template>
              {{this.collegeInfo.contactPhone}}
            </el-descriptions-item>
            <el-descriptions-item>
              <template slot="label"><i class="el-icon-s-custom"></i>学院logo</template>
              <img :src="this.collegeInfo.logo" width="80" height="80">
            </el-descriptions-item>
          </el-descriptions>
          ...
          <el-descriptions :column="1" size="50" border style="width: 35%;float: right;margin-right: 130px">
            ...
            <el-descriptions-item >
              <template slot="label"><i class="el-icon-s-custom"></i>电子邮箱</template>
              {{this.collegeInfo.email}}
            </el-descriptions-item>
            <el-descriptions-item class="long-content">
              <template slot="label"><i class="el-icon-s-custom" ></i>学院简介</template>
              <div>{{ this.collegeInfo.description }}</div>
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </el-dialog>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      collegeInfoDialogVisible:false,
  ...
  methods:{
    ...
    lookInfo(row){
      this.$axios({
        method:'GET',
        url:this.$httpUrl+'/university-information/listById?user_id='+row.id,
      }).then(res=>res.data).then(res=>{
        if(res.code == 200){
          this.univerInfo = res.data.univerInfo
        }else{
          this.univerInfo = ''
        }
      })
    ...
    lookCollegeInfo(row){
      this.$axios({
        method:'GET',
        url:this.$httpUrl+'/college/listById?user_id='+row.id,
      }).then(res=>res.data).then(res=>{
        if(res.code == 200){
          this.collegeInfo = res.data.collegeInfo
        }else{
          this.collegeInfo = ''
        }
      })
      this.collegeInfoDialogVisible = true
    },
    //查看二级学院
    lookAffiliation(row){
      this.$axios({
        method:'GET',
        url:this.$httpUrl+'/user/listCollege?id='+row.id,
      }).then(res=>res.data).then(res=>{
        if(res.code == 200){
          this.collegeTableData = res.data
        }else {
          this.collegeTableData = ''
        }
      })
    ...
    loadPost(){
      this.$axios({
        method:'POST',
        url:this.$httpUrl+'/user/listPageUniver',
        data: {
